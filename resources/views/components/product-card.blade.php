...
<div class="glow-border rounded-xl overflow-hidden group relative {{ $featured ? 'col-span-2 row-span-2' : '' }}"
    ...
    <div class="relative overflow-hidden aspect-square">
        ...
        <button type="button"
                ...
                class="absolute top-3 right-{{ isset($product->discount_price) && $product->discount_price < $product->price ? '12' : '3' }} z-10 p-1.5 rounded-full backdrop-blur-sm hover:bg-black/30 transition-all"
        ...
        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
            <a href="{{ route('products.show', $product->id) }}"
               class="font-semibold px-6 py-2.5 rounded-xl transition-all duration-300 transform translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100"
    ...
    <div class="p-4 space-y-3">
        <h3 class="font-heading font-semibold text-lg truncate" style="color: var(--text-primary);">{{ $product->name }}</h3>
        ...
        <div class="flex items-baseline gap-2">
            ...
            @else
                <span class="font-heading font-bold text-xl" style="color: var(--text-primary);">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
            @endif
        </div>
        ...
        <form method="POST" action="{{ route('cart.add') }}" class="flex items-center gap-2">
            ...
            <div class="flex items-center border border-accent/30 rounded-xl overflow-hidden">
                <button type="button" x-on:click="$el.nextElementSibling.value = Math.max(1, parseInt($el.nextElementSibling.value) - 1)"
                    class="px-3 py-2 text-text-muted hover:text-accent hover:bg-accent/10 transition-colors text-sm">-</button>
                <input type="number" name="quantity" value="1" min="1" max="{{ $product->stock }}"
                    class="w-12 text-center bg-transparent text-sm py-2 border-x border-accent/30 focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    style="color: var(--text-primary);">
                <button type="button" x-on:click="$el.previousElementSibling.value = Math.min({{ $product->stock }}, parseInt($el.previousElementSibling.value) + 1)"
                    class="px-3 py-2 text-text-muted hover:text-accent hover:bg-accent/10 transition-colors text-sm">+</button>
            </div>
            <button
                type="submit"
