    ...
    <div class="max-w-2xl glow-border bg-surface rounded-xl p-6">
        <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                @if ($product->image)
                    <div class="mb-3">
                        <img src="{{ asset($product->image) }}" class="w-40 h-40 object-cover rounded-xl border border-accent/20">
                    </div>
                @endif
                <input type="file" name="image" accept="image/jpeg,image/png,image/webp"
                       onchange="if(this.files[0]) document.getElementById('preview').src = URL.createObjectURL(this.files[0]); this.closest('div').querySelector('#preview').classList.remove('hidden')"
